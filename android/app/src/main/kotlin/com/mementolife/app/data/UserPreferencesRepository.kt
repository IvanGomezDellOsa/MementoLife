...
private val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "mementolife_prefs")
...
class UserPreferencesRepository(private val context: Context) {
    ...
    suspend fun setBirthDate(date: LocalDate) {
        context.dataStore.edit { it[Keys.BIRTH_DATE] = date.toString() }
    }

    suspend fun setLifeYears(years: Int) {
        context.dataStore.edit { it[Keys.LIFE_YEARS] = years.coerceIn(MIN_LIFE_YEARS, MAX_LIFE_YEARS) }
    }
    ...
    suspend fun setLocale(locale: AppLocale) {
        context.dataStore.edit { it[Keys.LOCALE] = locale.name }
    }

    suspend fun setEfemerideEnabled(enabled: Boolean) {
        context.dataStore.edit { it[Keys.EFEMERIDE_ENABLED] = enabled }
    }

    suspend fun setLastAppliedDate(date: LocalDate) {
        context.dataStore.edit { it[Keys.LAST_APPLIED_DATE] = date.toString() }
    }
}
